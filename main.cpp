...
void ScanConvertTriangle(
    ...
    //input are different oreder .. you can
    ...
    
{
    ...
    float l03_y, l03_m;
    float l03_r, l03_g, l03_b, l03_mr, l03_mg, l03_mb;
    int x;
    ...
    // // Computes the slopes for L03
    l03_m = (float)(y2 - y1) / (x2 - x1);
    l03_mr = (float)(r2 - r1) / (x2 - x1);
    l03_mg = (float)(g2 - g1) / (x2 - x1);
    l03_mb = (float)(b2 - b1) / (x2 - x1);
    ...
    for (x = x0; x < x1; x++) {
        ...
        //copy and paste to do right R
        // this ocde is the left R
        l01_y += l01_m;
        l01_r += l01_mr;
        l01_g += l01_mg;
        l01_b += l01_mb;
    ...
    // TODO: handle the case when x1==x2 --- add the second algorithm
    for (x = x1; x < x2; x++) {
        if (l03_y >= l02_y) {
...
void mousebuttonhandler(int button, int state, int x, int y)
{
    ...
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        // for 3 click check order to the input point
        if (cnt == 3) {
            // TODO: make sure point0, 1, 2 are passed in the correct order x0<=x1<=x2
            if (points[0][0] <= points[1][0]) {
                if (points[1][0] <= points[2][0]) { // x0 <= x1 <= x2 (that mean  this line)
                    ScanConvertTriangle(//if condition fail
                        // oimput poit are incorrect order

                        // 0 the first click and 1 the seconf right click and third is the # 2 click
                    ...
                    if (points[0][0] <= points[2][0]) { // x0 <= x2 <= x1
                        // pass 0 1 2 order!!!
                        ScanConvertTriangle(
                            points[0][0], points[0][1], color[0][0], color[0][1], color[0][2],  // x0, y0, r0, g0, b0 // pass x y
                            points[2][0], points[2][1], color[2][0], color[2][1], color[2][2],  // x2, y2, r2, g2, b2
                            points[1][0], points[1][1], color[1][0], color[1][1], color[1][2]   // x1, y1, r1, g1, b1
                        );
                    }
                        ...
                        ScanConvertTriangle(
                            //pass point 2 first pass input  point to the corrct order  pass ---0 2 1---
                            points[2][0], points[2][1], color[2][0], color[2][1], color[2][2],  // x2, y2, r2, g2, b2
                            points[0][0], points[0][1], color[0][0], color[0][1], color[0][2],  // x0, y0, r0, g0, b0
                            points[1][0], points[1][1], color[1][0], color[1][1], color[1][2]   // x1, y1, r1, g1, b1
                        );
           ...
           // TODO: fill in the rest here
            if (points[1][0] <= points[0][0]) {
                if (points[0][0] <= points[2][0]) { // x0 <= x1 <= x2 (that mean  this line)
                    ScanConvertTriangle(//if condition fail
                        // oimput poit are incorrect order
                    ...
                    if (points[1][0] <= points[2][0]) { // x0 <= x2 <= x1
                        // pass 0 1 2 order!!!
                        ScanConvertTriangle(
                            points[1][0], points[1][1], color[1][0], color[1][1], color[1][2],  // x0, y0, r0, g0, b0 // pass x y
                            points[2][0], points[2][1], color[2][0], color[2][1], color[2][2],  // x2, y2, r2, g2, b2
                            points[0][0], points[0][1], color[0][0], color[0][1], color[0][2]   // x1, y1, r1, g1, b1
                        );
                    }
